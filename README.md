...
- **购物车批量预约**: 将物品加入购物车，统一设置预约时间，批量提交预约，购物车以弹窗形式展示；购物车按当前浏览仓库隔离，切换仓库只切换购物车视图，不清空其他仓库的物品；外来物品归入当前浏览仓库的购物车，结算时后端校验用户仍是该仓库成员；设置时间后自动检测预约冲突并提示冲突时间段；预约单标题可编辑，默认为"用户名+的预约单#+日期简写"格式（如`张三的预约单#0310`），点击编辑图标可修改标题
