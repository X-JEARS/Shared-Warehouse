...
- **Cart**: Client-side only, persisted to localStorage via Zustand. Each `CartItem` records the current browsing `roomId`; `CartPopup`, the FAB badge, and checkout filter items to the current room, so switching rooms preserves independent cart views without clearing or prompting. Foreign items use the current browsing room's ID. Checkout sends `roomId`, and the backend verifies current room membership before creating the order. A successful checkout removes only that room's items. Persist version 2 drops legacy items without `roomId`. The store also includes `orderTitle` (editable order title, default generated as `用户名+的预约单#+日期简写`) and `setOrderTitle`; a manual `clearCart` still resets all items, title, and time.
...
- Pages using this pattern: MyItems, MyReservations, ReservationOrders, ReservationOrderDetail, RoomSettings, Notifications, MyProfile, AddBox, CreateItem, CreateRoom, JoinRoom, BoxDetail, SystemSettings
...
### UI Components
- **ItemCard**: Vertical layout card with image on top (56x56px), item name below, then tags. Stock status badge (在库/离库/外来物品) at top-right corner of card. Accepts `showStockStatus` prop to toggle status display and `showCartButton` to show a 22px circular cart toggle at bottom-right: a light-blue "+" adds the item, then becomes a gray "-" that removes it from the cart when clicked.
- **FilterBar**: Box/tag filters. Boxes are shown as a horizontally scrollable tab row matching the ReservationOrders tab style, with "全部", "不在库中" (shows only out-of-stock items), and individual boxes. A single measured blue indicator interpolates between tab positions during Warehouse swipes, and the destination title becomes active when the swipe reaches it. Tags are selected from a narrow, independently scrollable drawer opened by a pill-shaped FAB at the bottom-left. The drawer is constrained to the Warehouse content area so it does not cover the page header or MainLayout navigation. It lists only the room's tags (no "全部" option); when a tag is active, a text-only "取消筛选" action aligned with the FAB appears at the drawer bottom. The FAB displays "标签" with a right chevron when unfiltered and the selected tag name when active.
...
- **Warehouse page**: Items displayed in adaptive grid (`repeat(auto-fill, minmax(150px, 1fr))`). In-stock items grouped by `current_box`, items within each group sorted by name (`localeCompare` with i18n-aware locale: `i18n.language === 'en-US' ? 'en' : 'zh'`). Out-of-stock items sorted by name and displayed in "不在库中" section. Foreign items (from other rooms) shown with green "外来物品" badge. Box switching renders previous/current/next filter pages in a three-pane track so pointer drags and horizontal trackpad gestures move content continuously before sharing the same settle animation. Trackpad `deltaX` maps to drag pixels at a 1:1 ratio; new gesture detection completes an interrupted transition before applying the next movement, while horizontal browser-history navigation is suppressed inside the Warehouse content area.
...
- **CartPopup**: The only cart UI; the former standalone `Cart.tsx` page and `/cart` route were removed. It slides up from the Warehouse page like ItemDetail and receives the current `roomId`, filtering display, conflict checks, and checkout to that room. The fixed footer contains the confirm button, with scrollable content above. Items use a two-column grid (item image + item name + delete icon). It automatically checks reservation conflicts when time is set and shows the conflicting item and time range. The header shows an editable order title (default: `用户名+的预约单#+日期简写`, e.g. `张三的预约单#0310`) with the blue outline edit icon. Checkout submits both `roomId` and `title`; after success, only the current room's items are removed.
...
### Skeleton Loading
- Composite skeletons in `client/src/components/skeleton/` are exported via the barrel `index.ts`. Shared shape components are `ItemCardSkeleton`, `DetailSkeleton`, `OrderSkeleton`, and `FormSkeleton`; `PageSkeletons.tsx` contains layout-specific skeletons for MyItems, Notifications, MyTransferRecords, and ReservationOrderDetail. They compose antd-mobile's built-in `Skeleton` primitive and styled-components — no custom shimmer or `SkeletonBase`. Rounded corners use `var(--app-radius-*)` so they adapt to default/rounded/compact styles.
- `client/src/hooks/useMinLoadingTime.ts` (`useMinLoadingTime(loading, minMs=300)`) keeps a skeleton on screen for at least 300ms to avoid flicker on fast loads. Use it for detail-style pages (e.g. ItemDetail) where the skeleton summary would flash otherwise.
- Page-specific skeleton use: Warehouse/InHand use `ItemCardSkeleton` inside the same responsive item grid as real cards; Warehouse also keeps the group-title row. MyItems uses `MyItemListSkeleton`; Notifications uses `NotificationListSkeleton`; MyTransferRecords uses `TransferRecordListSkeleton`; MyReservations/ReservationOrders use `OrderSkeleton`; ReservationOrderDetail uses `ReservationDetailSkeleton`; BoxDetail and RoomSettings compose skeleton blocks inside their real page cards/grids; CreateItem replaces the asynchronous storage and tag selectors with `FormSkeleton`; ItemDetail uses `DetailSkeleton` in the real fixed-summary container and section-shaped placeholders in the real scrollable-details container.
- Loading branches must reuse the real page containers, padding, tabs/search bars, and grid definitions. Do not add a second wrapper padding or substitute a different card orientation, because that causes horizontal shifts and responsive column mismatches when data arrives.
- SWR-backed read pages set `keepPreviousData: true` so refresh does not re-trigger the skeleton; only a true first load (or a route-`id` change in BoxDetail/ReservationOrderDetail, where `setLoading(true)` lives in the `[id]` effect) shows the skeleton — business refreshes must not re-flip loading to avoid skeleton on every refetch.
- Pages intentionally without skeleton: Scanner (the overlay is camera init, not data loading) and CartPopup (synchronous Zustand read, no loading state).
- `theme.css` overrides `.adm-skeleton` for dark-mode contrast and disables animation under `@media (prefers-reduced-motion: reduce)`.
- `App.tsx` lazy-route `PageFallback` shows a branded fallback (PWA `/icons/icon-192.png` + primary `SpinLoading`) instead of a bare spinner.
...
- FABs: Tag filter pill at bottom left (always visible when a room is selected) and Cart button at bottom right (only visible when the current room's cart has items; its badge is also scoped to the current room)
