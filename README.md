...
- **批量取走/放入**: 扫码后物品自动加入待操作列表，支持连续扫描多个物品，一键批量确认取走或放入；物品去重防止重复扫描，已在手中的物品标记排除
...
- **我的页面菜单**: 菜单项使用 antd-mobile-icons 线框图标，分为三组：我的资料（UserOutline）+ 我的物品（AppstoreOutline）+ 我的预约（CalendarOutline）为第一组，系统设置（SetOutline）为第二组（与第一组之间有空隙分隔），关于（InformationCircleOutline）为第三组
...
- **系统设置**: 支持配色模式切换（浅色/深色/跟随系统）和视觉风格切换（标准/圆润/紧凑），通过 CSS 变量体系实现全局主题适配，包括 Ant Design Mobile 组件颜色覆盖；系统语言功能暂未开放
...
- **预约订单管理**: 仓库预约页面支持通过顶部下拉栏切换仓库查看不同仓库的预约订单，右侧搜索按钮可按预约标题筛选；我的预约页面显示个人预约，支持取消单个物品预约或整个订单；预约卡片顶部显示订单标题与状态标签，分隔线下方为预约时间（加粗），再下方为物品数量；订单创建者可在详情页编辑订单标题（蓝色线框编辑按钮）和延长订单（选择新结束时间，所有还在预约状态中的物品自动延长）；订单详情页的预约物品列表支持卡片/清单两种视图切换，清单视图每行仅显示物品名和圆点（绿色=在手中，灰色=不在）
...
- `POST /api/scan/borrow-batch` - 批量取走物品（支持部分成功）
- `POST /api/scan/return` - 将单个物品放入指定盒子（无需持有物品即可操作）
- `POST /api/scan/return-batch` - 批量将物品放入指定盒子（支持部分成功）
