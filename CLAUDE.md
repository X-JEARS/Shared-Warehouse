...
- **Reservation Order Detail**: Located at `client/src/pages/ReservationOrderDetail.tsx`, uses unified sub-page Header with ← back button.
  ...
  - **Reservation items**: Displayed in two-column grid (`ReservationGrid`, `grid-template-columns: repeat(2, 1fr)`). Each card shows item name + status tag, location, time range (font-size: 12px, color: #333, font-weight: 500, separated by border-top). No calendar emoji before time.
